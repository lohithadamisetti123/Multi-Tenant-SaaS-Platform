...
exports.createTask = async (req, res) => {
  try {
    ...
    if (req.body.assignedTo) {
      ...
      const user = await User.findOne({ where: { id: req.body.assignedTo, tenantId: req.user.tenantId } });
      if (!user) return res.status(400).json({ success: false, message: 'Assigned user not found in this workspace' });
    }
...
exports.updateTask = async (req, res) => {
  try {
    ...
    if (assignedTo) {
      const { User } = require('../models');
      const user = await User.findOne({ where: { id: assignedTo, tenantId: req.user.tenantId } });
      if (!user) return res.status(400).json({ success: false, message: 'Assigned user not found in this workspace' });
    }
